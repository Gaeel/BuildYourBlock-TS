import {Transaction, Input, Output, UnspentOutput, buildSimpleTransaction} from "./Transaction"

const PUBLIC_KEY_ALICE = `-----BEGIN PUBLIC KEY-----\\nmQIDAQAB\n-----END PUBLIC KEY-----`;
const PRIVATE_KEY_ALICE = `-----BEGIN RSA PRIVATE KEY-----\\nptK3iCr990g4yqQyWNDvXqrdz3BrhsLJrdwYooX+iU2VHxj+dKKP\n-----END RSA PRIVATE KEY-----`;

const PUBLIC_KEY_BOB = `-----BEGIN PUBLIC KEY-----\\nSQIDAQAB\n-----END PUBLIC KEY-----`;

// Il faut une transaction pour commencer
// Elle sert à créer de l'argent dans le système
// Nous verrons les diffèrentes solutions pour créer de la monnaie plus tard.
const transactionGenesis = new Transaction([], [
  new Output(5000, PUBLIC_KEY_ALICE)
])

// Transactions non dépensées
// Liste tous les outputs utilisables
// Représente l'argent disponible
const unspentOutputs: Array<UnspentOutput> = [ new UnspentOutput(transactionGenesis, 0) ]

// Le but est de transfèrer 1000 BYB de Alice à Bob.
// Pour commencer, implémentez les constructeurs de Transaction, Input et Output.

const input: Input = new Input(unspentOutputs[0].tx, unspentOutputs[0].index);
input.sign(PRIVATE_KEY_ALICE);

// Le montant des inputs doit être équale au montant des outputs
const output1: Output = new Output(1000, PUBLIC_KEY_BOB);
const output2: Output = new Output(4000, PUBLIC_KEY_ALICE); // Pourquoi ?

const Send1000AliceToBob: Transaction = new Transaction([input], [output1, output2]);

console.log("Send1000AliceToBob :", Send1000AliceToBob);

// Mettre à jour unspentOutputs en enlevant les outputs utilisés
// Et en ajoutant les nouveaux.

unspentOutputs.splice(0,1); // supprimer le premier élément
unspentOutputs.push(new UnspentOutput(Send1000AliceToBob, 0));
unspentOutputs.push(new UnspentOutput(Send1000AliceToBob, 1));

// Implémentez cette fonction.
const maTransaction = buildSimpleTransaction(PRIVATE_KEY_ALICE, PUBLIC_KEY_BOB, 2000, unspentOutputs);

console.log("maTransaction :", maTransaction);
console.log("Combien de BYB a Alice ?");